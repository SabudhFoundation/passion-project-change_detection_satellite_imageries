...
def _nspp_block(f_enc):
    ...
    for stride in [2, 4, 8, 16]:
        ...
        b_avg = layers.Conv2D(
            c4, 1, padding="same", name=f"nspp_avg_pw_s{stride}"
        )(b_avg)
        ...
        fg = layers.Conv2D(
            c4, 1, padding="same", name=f"nspp_gpw_s{stride}"
        )(fg)
    ...
    return layers.Conv2D(
        C, 1, padding="same", activation="relu", name="nspp_out"
    )(cat)
...
def build_ucdnet(input_shape=(512, 512, 13), num_classes=2):
    ...
    output = layers.Conv2D(
        ...
        activation="softmax", name="change_map"
    )(d3)
